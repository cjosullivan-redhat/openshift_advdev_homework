...
oc set probe dc/tasks-blue --readiness --get-url=http://:8080/ --initial-delay-seconds=60 --timeout-seconds=1 -n ${GUID}-tasks-prod
oc set probe dc/tasks-blue --liveness --get-url=http://:8080/ --initial-delay-seconds=60 --timeout-seconds=1 -n ${GUID}-tasks-prod
